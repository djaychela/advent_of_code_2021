...
possible = permutations(range(len(homework)), 2)

results = {}

for poss in tqdm(possible):
    hw = parse(homework[poss[0]])
    hw_2 = parse(homework[poss[1]])
    hw = reduce_homework(hw)
    hw.insert(0, "[")
    hw.extend(hw_2)
    hw.append("]")
    hw = reduce_homework(hw)
    score = calculate_overall_magnitude(hw)

    results[f"{poss[0]}+{poss[1]}"] = score

print(f"Maximum Magnitude: {max(results.values())}")
